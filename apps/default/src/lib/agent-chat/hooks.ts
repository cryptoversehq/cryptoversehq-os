 ...
 *   const { sendMessage, messages, error } = useAgentChat('agent-456', conversationId);
 *
 *   // Create conversation manually. Always try/catch so a failure stays visible.
 *   const handleStartChat = async () => {
 *     try {
 *       const { conversationId: newId } = await createConversation('agent-456');
 *       setConversationId(newId);
 *     } catch (err) {
 *       // surface the failure to the user (do not leave the button dead)
 *     }
 *   };
 *
